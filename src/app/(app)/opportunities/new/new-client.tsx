          ...
          <div className="space-y-8">
            ...
            <FormSection number="03" title="Planning en budget" description="Alleen invullen wat al bekend is.">
              <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                <Field label="Startdatum"><input type="date" value={form.startDate} onChange={(event) => setField("startDate", event.target.value)} className="input" /></Field>
                <Field label="Einddatum"><input type="date" value={form.endDate} min={form.startDate || undefined} onChange={(event) => setField("endDate", event.target.value)} className="input" /></Field>
                <Field label="Reageren vóór"><input type="date" value={form.responseDeadline} min={new Date().toISOString().slice(0, 10)} onChange={(event) => setField("responseDeadline", event.target.value)} className="input" /></Field>
